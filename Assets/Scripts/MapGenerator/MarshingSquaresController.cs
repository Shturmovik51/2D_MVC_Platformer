...
namespace Platformer2D
{
    public class MarshingSquaresController
    {
        ...
        public void DrawTilesOnMap(Tilemap tileMapGround, Tile groundTile, Tile grassTile)
        {
            if (_squareGrid == null)
            {
                return;
            ...
            for (int x = 0; x < _squareGrid.Squares.GetLength(0); x++)
            {
                ...
                {
                    DrawTile(_squareGrid.Squares[x, y].TopLeft.Active, _squareGrid.Squares[x, y].TopLeft.Position, _groundTile);
                    DrawTile(_squareGrid.Squares[x, y].TopRight.Active, _squareGrid.Squares[x, y].TopRight.Position, _groundTile);

                    DrawTile(_squareGrid.Squares[x, y].BottomLeft.Active, _squareGrid.Squares[x, y].BottomLeft.Position, _groundTile);
                    DrawTile(_squareGrid.Squares[x, y].BottomRight.Active, _squareGrid.Squares[x, y].BottomRight.Position, _groundTile);
                }
            }
        }

        public void DrawTile(bool active, Vector3 position, Tile tile)
        {
            if (active)
            {
